...
├── resource-monitor.service.ts          # 资源监控器 ✅
└── index.ts                             # 模块导出 ✅
...
- ✅ **任务 9.1**: 实现 ResourceMonitor 服务
- ✅ **任务 9.2**: 集成资源监控到 ParallelProcessingManager
...
**主要功能:**
- 协调整个并行处理流程（5 个步骤）
- 集成 ChunkSplitter、WorkerPool、ResultCollector、ProgressTracker、PerformanceMonitor、ResourceMonitor
- 文件分割和数据块分配
- 并行任务执行和监控
- 超时处理（默认 5 分钟）
- 错误处理和部分结果收集
...
- 实时进度和性能查询
- 完整的状态管理
- **资源限制控制**：在内存超限时自动暂停工作线程创建，等待内存释放

**使用示例:** 参见 `parallel-processing-manager.example.ts`

### 8. ResourceMonitor (资源监控器)

监控系统资源使用情况并实施资源限制，确保系统稳定运行。
...
**使用示例:** 参见 `resource-monitor.example.ts`
...
2. **步骤 2/5: 初始化组件** - 初始化工作线程池、结果收集器、进度跟踪器、性能监控器、资源监控器
3. **步骤 3/5: 创建任务** - 为每个数据块创建 WorkerTask
4. **步骤 4/5: 并行执行** - 使用 WorkerPool 并行执行所有任务，监控进度、性能和资源使用
   - 在创建每个工作线程前检查资源限制
   - 如果内存超限，暂停并等待内存释放（最多 30 秒）
   - 记录资源使用情况
5. **步骤 5/5: 收集结果** - 聚合所有工作线程结果，生成性能报告

## 资源管理集成

ParallelProcessingManager 已集成 ResourceMonitor，提供以下资源管理功能：

1. **自动资源监控**：处理开始时自动启动资源监控
2. **内存限制控制**：默认最大内存 1800MB，警告阈值 1500MB
3. **智能暂停机制**：内存超限时自动暂停工作线程创建
4. **等待内存释放**：暂停后等待内存释放（最多 30 秒）
5. **资源使用记录**：记录每个工作线程创建前的资源状态
6. **自动清理**：处理完成或失败时自动停止资源监控

这确保了系统在处理大文件时保持稳定，符合需求 7（资源管理和内存控制）和属性 7（内存使用边界）。

## 下一步

继续执行任务 9.3：为资源监控编写单元测试
